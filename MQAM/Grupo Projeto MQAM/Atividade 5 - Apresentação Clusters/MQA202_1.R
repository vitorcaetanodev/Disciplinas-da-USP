...
#normalizando a base
base_clus <- as.data.frame(na.omit(scale(MQA)))
View(base_clus)

#Dendrograma
notasclust <- hclust(d = dist(base_clus, method = 'euclidean'), method = 'centroid')
plot(notasclust)
##baseado no grafico, escolhemos 4 clusters 

##pegando os grupos previstos, com corte da ?rvore em 4 grupos
